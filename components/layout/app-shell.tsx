...
function NavIcon({ icon, className = "" }: { icon: (typeof navItems)[number]["icon"]; className?: string }) {
  const props = { className };
  if (icon === "message-circle") return <MessageCircle {...props} />;
  if (icon === "notebook-pen") return <NotebookPen {...props} />;
...
export function AppShell({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="app-shell flex bg-[var(--background)]">
      <aside
        className={`hidden border-r border-[var(--border)] bg-[var(--panel)] transition-all duration-300 md:flex md:flex-col ${collapsed ? "md:w-20" : "md:w-64"}`}
      >
        ...
        <nav className="space-y-1 px-2">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`focus-ring flex items-center gap-3 rounded-xl px-3 py-2 text-sm transition-colors duration-200 ${
      ...
      <div className="flex min-h-dvh flex-1 flex-col">
        ...
        <nav className="safe-bottom fixed inset-x-0 bottom-0 z-20 border-t border-[var(--border)] bg-[var(--panel)] backdrop-blur md:hidden">
      ...
      {drawerOpen ? (
        <div className="fixed inset-0 z-30 bg-black/40 transition-opacity duration-200 md:hidden" onClick={() => setDrawerOpen(false)}>
          <aside className="h-full w-64 animate-slide-in bg-[var(--panel)] p-4 shadow-[var(--shadow-md)]" onClick={(e) => e.stopPropagation()}>
              ...
              {navItems.map((item) => (
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-xl px-3 py-2 transition-colors duration-200 ${
                    pathname === item.href ? "bg-[var(--panel-2)]" : ""
                  }`}
                >
                  <NavIcon icon={item.icon} className="h-4 w-4" />
                  <span>{item.label}</span>
